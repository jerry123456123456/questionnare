...
bool CDBConn::ExecuteCreate_test(const char *sql_query) {
    mysql_ping(mysql_);
    ...
    if (mysql_real_query(mysql_, sql_query, strlen(sql_query))) {
        ...
        // 获取执行INSERT操作后受影响的行数等信息
        query_result = mysql_affected_rows(mysql_);
    }

    // 根据不同的结果状态进行相应处理并记录相关信息
    if (query_result == 0) {
        LogWarn("INSERT操作执行，但未影响任何行");
    } else if (query_result > 0) {
        LogInfo("INSERT操作成功，影响了 {} 行", query_result);
    } else {
        LogError("INSERT操作出现未知错误");
    }
